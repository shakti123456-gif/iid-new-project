...
class CheckoutView_subscription(LoginRequiredMixin, View):
    login_url = '/login/'
    ...
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            coupon=kwargs['pk']
            check=UserMembership.objects.filter(usermembership__email=self.request.user).first()
            member=membership.objects.get(membership_type=coupon)
            form = CheckoutForm()
            ...
            return render(self.request, "app/subsciption.html", context)

                 
        except ObjectDoesNotExist:
            messages.info(self.request, "subscription page ")
            return redirect("app:home")
        ...
        try:
            ...
            if form.is_valid():
                ...
                if payment_option == 'PU':
                    ...
                    initial.update({'key': settings.PAYU_MERCHANT_KEY,
                                    'surl': self.request.build_absolute_uri(reverse('app:order.success1')),
                                    'furl': self.request.build_absolute_uri(reverse('app:order.failure')),
                                    'curl': self.request.build_absolute_uri(reverse('app:order.cancel')),
                                    'hash': hashh,
                                    'service_provider': 'payu_paisa',
...
@csrf_protect
@csrf_exempt
def successsubcription(request):
    if request.method == "POST":
        if True :
            email = request.POST["email"]
            check=UserMembership.objects.filter(usermembership__email=email)[0]
            ...
            for i in membership.objects.all():
                ...
                if float(i.price) ==float(request.POST["amount"]):
                 
                    check.membership_method=i
                    check.save()
